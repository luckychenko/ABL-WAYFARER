...
router.post('/signup', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ status: 'success', data: { user_id: user.id, is_admin: user.is_admin, token } });
  } catch (err) {
  ...
  if (email && password) {
    try {
      ...
      if (bcrypt.compareSync(password, user.rows[0].password)) {
        const token = tokenService().issue(user.rows[0]);

        return res.status(200).json({ status: 'success', data: { user_id: user.rows[0].id, is_admin: user.rows[0].is_admin, token } });
      }
